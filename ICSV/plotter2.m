...
%	Do averaging for post-processing	
	...
	if (plot_outlet_pi_signal)
		d1 = figure();
		set(d1, 'Position', [0 0 500 350]);
		d2 = figure();
		set(d2, 'Position', [0 0 500 350]);
		d3 = figure();
		set(d3, 'Position', [0 0 500 350]);
		for probe = 1:Nprobes
			figure(d1);
			plot(t_smpl(:,probe), real(p_pi_MC(:,probe)), 'k-', 'LineWidth', 2);
			hold on;
			plot(t_smpl(:,probe), real(p_pi_DM(:,probe)), 'b-', 'LineWidth', 2);
			xlabel('t [ms]');
			if (probe == 1)
...
%			end
	...
	if (plot_outlet_p_signal)
		f1 = figure();
		set(f1, 'Position', [0 0 500 350]);
		f2 = figure();
		set(f2, 'Position', [0 0 500 350]);
		f3 = figure();
		set(f3, 'Position', [0 0 500 350]);
		for probe = 1:Nprobes
			figure(f1);
			...
			if (probe == 1)
				h = legend('Compact', 'Non-compact');
				set(h, 'FontSize', 14, 'Location', 'Southwest');
			end
