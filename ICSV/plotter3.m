...
%	Allocate some arrays
	...
	pi_minus_hat_2_DM		= zeros(N,Nprobes);						% Store the dimensionless downstream pressure wave computed with Duran-Moreau style model
	pi_minus_hat_2_MC		= zeros(N,Nprobes);
	p_phase_2_DM			= zeros(N,Nprobes);
	p_phase_2_MC			= zeros(N,Nprobes);
	p_minus_phase_2_DM		= zeros(N,Nprobes);
	p_minus_phase_2_MC		= zeros(N,Nprobes);
	pi_total_hat_DM			= zeros(N,Nprobes);
	pi_total_hat_MC			= zeros(N,Nprobes);
	p_2_hat_DM 				= zeros(N,Nprobes);						% Store the dimensional downstream pressure wave computed with Duran-Moreau style model
	p_2_hat_MC 				= zeros(N,Nprobes);						% Store the dimensional downstream pressure wave computed with Marble-Candel style model
...
%		Apply the transfer functions to the incoming data			
		for i = 1:N
			if (abs(He(i)) < 2)													% Only act on waves w He < 2
				...
				p_minus_phase_2_DM(i,probe)	= p_phase_1(i,probe) + ppval(minusphase, abs(He(i)));
				p_minus_phase_2_MC(i,probe)	= p_phase_1(i,probe) + ppval(minusphase, 0);

				pi_total_hat_DM(i,probe)	= pi_plus_hat_2_DM(i,probe)*(cos(p_phase_2_DM(i,probe)) + sqrt(-1)*sin(p_phase_2_DM(i,probe))) ...
											+ pi_minus_hat_2_DM(i,probe)*(cos(p_minus_phase_2_DM(i,probe)) + sqrt(-1)*sin(p_minus_phase_2_DM(i,probe)));
				pi_total_hat_MC(i,probe)	= pi_plus_hat_2_MC(i,probe)*(cos(p_phase_2_MC(i,probe)) + sqrt(-1)*sin(p_phase_2_MC(i,probe))) ...
											+ pi_minus_hat_2_MC(i,probe)*(cos(p_minus_phase_2_MC(i,probe)) + sqrt(-1)*sin(p_minus_phase_2_MC(i,probe)));

				p_2_hat_DM(i,probe) 	= pi_total_hat_DM(i,probe)*gamma*pbar_2;			% Scale the dimensionless output to make it dimensional MC		
				p_2_hat_MC(i,probe) 	= pi_total_hat_MC(i,probe)*gamma*pbar_2;			% Scale the dimensionless output to make it dimensional DM	
%				p_phase_2_MC(i,probe) = 0;
			end
		end

%		Build some arrays for output & plotting
		p_pi_hat_DM(:,probe)		= pi_total_hat_DM(:,probe);
		p_pi_hat_MC(:,probe)		= pi_total_hat_MC(:,probe);
...
%		Apply the transfer functions to the incoming data			
		for i = 1:N
			if (abs(He(i)) < 2)																	% Only act on waves w He < 2
				...
				pi_minus_hat_2_DM(i,probe)	= abs(sigma_hat_1(i,probe))*ppval(minusmodulus, abs(He(i)));
				pi_minus_hat_2_MC(i,probe)	= abs(sigma_hat_1(i,probe))*ppval(minusmodulus, 0);			% Stretch the modulus by the factor for He = 0

				p_phase_2_DM(i,probe)		= s_phase_1(i,probe) + ppval(phase, abs(He(i)));
				p_phase_2_MC(i,probe)		= s_phase_1(i,probe) + ppval(phase, 0);
				p_minus_phase_2_DM(i,probe)	= s_phase_1(i,probe) + ppval(minusphase, abs(He(i)));
				p_minus_phase_2_MC(i,probe)	= s_phase_1(i,probe) + ppval(minusphase, 0);
...
%				s_phase_2_MC(i,probe) = 0;
				...
				s_phase_2_MC(i,probe) = 0;
				p_2_hat_DM(i,probe) = 0;
				p_2_hat_MC(i,probe) = 0;
			end
		end

%		Build some arrays for output & plotting
		s_pi_hat_DM(:,probe)		= pi_total_hat_DM(:,probe);
		s_pi_hat_MC(:,probe)		= pi_total_hat_MC(:,probe);

		s_pi_DM(:,probe)			= ifft(fftshift(s_pi_hat_DM(:,probe)));
		s_pi_MC(:,probe)			= ifft(fftshift(s_pi_hat_MC(:,probe)));

		s_p_hat_DM(:,probe)			= p_2_hat_DM(:,probe);
		s_p_hat_MC(:,probe)			= p_2_hat_MC(:,probe);
%%%%		
...
%		Load the data from the Helmholtz dependent solver
		...
		minusmodulusTransferFn = abs(TRANS(:,2));
		phaseTransferFn = PHASE(:,1);							% Peel off the downstream acoustic transfer function from the PHASE array
		minusphaseTransferFn = PHASE(:,2);						% Peel off the downstream acoustic transfer function from the PHASE array
...
%		Apply the transfer functions to the incoming data			
		for i = 1:N
			if (abs(He(i)) < 2)																	% Only act on waves w He < 2
				...
				pi_minus_hat_2_DM(i,probe) 	= abs(xi_hat_1(i,probe))*ppval(minusmodulus, abs(He(i)));		% Stretch the modulus by the appropriate factor for current He
				pi_minus_hat_2_MC(i,probe)	= abs(xi_hat_1(i,probe))*ppval(minusmodulus, 0);				% Stretch the modulus by the factor for He = 0

				z_phase_2_DM(i,probe)		= z_phase_1(i,probe) + ppval(phase, abs(He(i)));
				z_minus_phase_2_DM(i,probe)	= z_phase_1(i,probe) + ppval(minusphase, abs(He(i)));
				z_phase_2_MC(i,probe)		= z_phase_1(i,probe) + ppval(phase, 0);
				z_minus_phase_2_MC(i,probe)	= z_phase_1(i,probe) + ppval(minusphase, 0);


				pi_total_hat_DM(i,probe)	= pi_plus_hat_2_DM(i,probe) *(cos(z_phase_2_DM(i,probe)) 		+ sqrt(-1)*sin(z_phase_2_DM(i,probe))) ...
											+ pi_minus_hat_2_DM(i,probe)*(cos(z_minus_phase_2_DM(i,probe)) 	+ sqrt(-1)*sin(z_minus_phase_2_DM(i,probe)));
				pi_total_hat_MC(i,probe)	= pi_plus_hat_2_MC(i,probe) *(cos(z_phase_2_MC(i,probe)) 		+ sqrt(-1)*sin(z_phase_2_MC(i,probe))) ...
											+ pi_minus_hat_2_MC(i,probe)*(cos(z_minus_phase_2_MC(i,probe)) 	+ sqrt(-1)*sin(z_minus_phase_2_MC(i,probe)));

				p_2_hat_DM(i,probe) 		= pi_total_hat_DM(i,probe)*gamma*pbar_2;			% Scale the dimensionless output to make it dimensional MC		
				p_2_hat_MC(i,probe) 		= pi_total_hat_MC(i,probe)*gamma*pbar_2;			% Scale the dimensionless output to make it dimensional DM	
			else
				z_phase_1(i,probe) = 0;
				pi_total_hat_DM(i,probe) = 0;
				pi_total_hat_MC(i,probe) = 0;
				p_2_hat_DM(i,probe) = 0;
				p_2_hat_MC(i,probe) = 0;
...
%		Build some arrays for output & plotting
		z_pi_hat_DM(:,probe)		= pi_total_hat_DM(:,probe);
		z_pi_hat_MC(:,probe)		= pi_total_hat_MC(:,probe);
...
%		z_p_hat_MC(:,probe)			= p_2_hat_amp_MC(:,probe);
		z_p_hat_DM(:,probe)			= p_2_hat_DM(:,probe);
		z_p_hat_MC(:,probe)			= p_2_hat_MC(:,probe);
...
%			ylim([-10000,10000]);
	...
	if (plot_good_SPL)
		...
		size(p_SPL_bar_MC)
		size(omega_smpl)
		plot(He, p_SPL_bar_MC, 'k-', 'lineWidth', 2);
		hold on;
		plot(He, p_SPL_bar_DM, 'b:', 'LineWIdth', 2);
...
%		title('Acoustic Forcing');
		...
		h = legend('Compact', 'Non-compact');
		set(h, 'FontSize', 14, 'Location', 'NorthEast');
		print -depsc p_SPL.eps

		h2 = figure();
		plot(He, s_SPL_bar_MC, 'k-', 'lineWidth', 2);
		hold on;
		plot(He, s_SPL_bar_DM, 'b:', 'LineWIdth', 2);
		xlabel('$He$', 'Interpreter', 'LaTeX');
		ylabel('SPL [dB]');
...
%		h = legend('Compact', 'Non-compact');
%		set(h, 'FontSize', 14, 'Location', 'NorthEast');
		print -depsc s_SPL.eps

		h3 = figure();
		plot(He, z_SPL_bar_MC, 'k-', 'lineWidth', 2);
		hold on;
		plot(He, z_SPL_bar_DM, 'b:', 'LineWIdth', 2);
		xlabel('$He$', 'Interpreter', 'LaTeX');
		ylabel('SPL [dB]');
...
%		h = legend('Compact', 'Non-compact');
%		set(h, 'FontSize', 14, 'Location', 'NorthEast');
		print -depsc z_SPL.eps

		h4 = figure();
		plot(He, t_SPL_bar_MC, 'k-', 'lineWidth', 2);
		hold on;
		plot(He, t_SPL_bar_DM, 'b:', 'LineWIdth', 2);
		xlabel('$He$', 'Interpreter', 'LaTeX');
		ylabel('SPL [dB]');
%		title('Total Forcing');
		xlim([0, 2]);
