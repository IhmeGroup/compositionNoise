function[] = plotTemperatureSensitivity()
...
%		psi_a = returnInertPsi(T_a, p_a, Ylft, Yctr, Yrgt, Zlft, Zctr, Zrgt, a, A, MW, Rbar);
	...
	Zratio = abs(Zratio);
	hh = figure();
	surface(MB, TA, Zratio,'edgecolor','none');
	shading interp;
	h = colorbar('northoutside');
%	set(h, 'FontSize', fs, 'FontName' 'Times');
	colormap('jet');
	xlabel('$M_c$','Interpreter','LaTeX', 'FontSize', fs, 'FontName', 'Times');
	ylabel('$T_a$ [K]', 'Interpreter', 'LaTeX', 'FontSize', fs, 'FontName','Times');
	set(gca,'FontSize', fs, 'FontName','Times');
	xlim([0, 2]);
	ylim([300,2000]);

	print -djpeg InertTemperatureSensitivity.jpg
end%plotTemperatureSensitivity()
